...
Resources (owned by des.py, named `s{stage}`):
  * `{prefix}.l{i}.cw` / `{prefix}.l{i}.ccw` -- member i's clockwise /
    counter-clockwise outbound link (separate resources = full duplex,
    matching a Link's bandwidth being per-direction).  Bandwidth occupancy
    only.
...
def ring_allreduce(
    ...
    In every step each member `i` sends `payload/g` bytes to `(i+1) % g` on
    its clockwise outbound link -- one task per member per step carrying only
    the bandwidth occupancy `payload/(g*bw)`.  A barrier task carrying the
    propagation latency `lat` joins each step to the next (the next step's
    sends wait on *all* of this step's sends, then one flight time).  Returns
    the final barrier key.  In isolation the makespan is exactly
    ...
    for _ in range(steps):
        ...
        sends = [
            _add(tasks, _cw(prefix, i), occ, list(deps), label)
...
def _a2a_switched(
    ...
    for i in range(group):
        ...
        for _ in range(group - 1):
            ...
            prev = _add(tasks, _cw(prefix, i), occ, deps, label)
...
def all_to_all(
    ...
    if topology is Topology.MESH_2D:
        ...
        # yet, so fall back to the closed form (occupancy on the link, latency
        # on a barrier -- same latency/occupancy split as the real expansions).
        deps = [entry] if entry is not None else []
        occ = _add(tasks, _cw(prefix, 0), comm_bytes / bw, deps, label)
...
def allreduce(
    ...
    if topology is Topology.MESH_2D:
        ...
        steps = 2 * (group - 1)
        deps = [entry] if entry is not None else []
        occ = _add(tasks, _cw(prefix, 0), steps / group * payload / bw, deps, label)
        return _add(tasks, f"{prefix}.bar{len(tasks)}", steps * lat, [occ], label)
    return ring_allreduce(tasks, entry, group, payload, bw, lat, prefix, label)
